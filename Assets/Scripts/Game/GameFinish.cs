...
public class GameFinish : MonoBehaviour {
    ...
    public Transition mainMenuTransition = Transition.CornerIn;
    public Transition quitGameTransition = Transition.BoxIn;

    private CameraControl camControl;
    private Score score;

    private Texture2D gameFinish;
    ...
    void Awake() {
        ...
        gameFinish = (Texture2D)Resources.Load("Textures/GameFinished");
        background = (Texture2D)Resources.Load("Textures/Background");
    }

    public void OnGUI() {
        if (gameFinish != null) {
            GUI.DrawTexture(new Rect(Screen.width / 2 - gameFinish.width / 2, Screen.height / 2 - 225, gameFinish.width, gameFinish.height), gameFinish);
        }
